...
def step_lorenz_mod_1(d, dt=0.005):
    global x, y, z
    ...
    # Euler integration step for the Lorenz Mod 1 Attractor
    dx = -a * x + y**2 - z**2 + a * c
    dy = x * (y - b * z) + d
    dz = -z + x * (b * y + z)
...
def draw():
    ...
    d = 0.08 + 0.05 * np.sin(t)
    
    # Run integration steps
    for _ in range(3):
        step_lorenz_mod_1(d, dt=0.005)
    ...
    r_col[coral_mask] = np.maximum(r_col[coral_mask], 10 + 245 * ((density_norm[coral_mask] - 0.3) / 0.4))
    g_col[coral_mask] = np.maximum(g_col[coral_mask], 35 + 95 * ((density_norm[coral_mask] - 0.3) / 0.4))
    b_col[coral_mask] = np.maximum(b_col[coral_mask], 40 + 70 * ((density_norm[coral_mask] - 0.3) / 0.4))
    
    # Amber highlights
    amber_mask = density_norm > 0.7
    r_col[amber_mask] = np.maximum(r_col[amber_mask], 255)
    g_col[amber_mask] = np.maximum(g_col[amber_mask], 130 + 65 * ((density_norm[amber_mask] - 0.7) / 0.3))
    b_col[amber_mask] = np.maximum(b_col[amber_mask], 110 - 110 * ((density_norm[amber_mask] - 0.7) / 0.3))
