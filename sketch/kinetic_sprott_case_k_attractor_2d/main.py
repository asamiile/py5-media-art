...
def step_sprott_k(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case K Attractor
    dx = x * y - z
    dy = x - y
    dz = x + a * z
...
def draw():
    ...
    # Map to screen
    # Sprott Case K typical size: x,y,z in [-5, 5]
    ...
    b_col = 28 * (density_norm ** 1.5)
    
    # Ruby midtones
    ruby_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[ruby_mask] = np.maximum(r_col[ruby_mask], 25 + 195 * ((density_norm[ruby_mask] - 0.3) / 0.4))
    g_col[ruby_mask] = np.maximum(g_col[ruby_mask], 25 + 10 * ((density_norm[ruby_mask] - 0.3) / 0.4))
    b_col[ruby_mask] = np.maximum(b_col[ruby_mask], 28 + 20 * ((density_norm[ruby_mask] - 0.3) / 0.4))
    
    # Pearl highlights
    pearl_mask = density_norm > 0.7
    r_col[pearl_mask] = np.maximum(r_col[pearl_mask], 220 + 35 * ((density_norm[pearl_mask] - 0.7) / 0.3))
    g_col[pearl_mask] = np.maximum(g_col[pearl_mask], 35 + 215 * ((density_norm[pearl_mask] - 0.7) / 0.3))
    b_col[pearl_mask] = np.maximum(b_col[pearl_mask], 48 + 207 * ((density_norm[pearl_mask] - 0.7) / 0.3))
