...
def step_sprott_i(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case I Attractor
    dx = a * y
    dy = x + z
    dz = x + (y ** 2) - z
...
def draw():
    ...
    # Map to screen
    # Sprott Case I typical size: x,y,z in [-5, 5]
    ...
    # Silver midtones
    silver_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[silver_mask] = np.maximum(r_col[silver_mask], 15 + 160 * ((density_norm[silver_mask] - 0.3) / 0.4))
    g_col[silver_mask] = np.maximum(g_col[silver_mask], 15 + 160 * ((density_norm[silver_mask] - 0.3) / 0.4))
    b_col[silver_mask] = np.maximum(b_col[silver_mask], 15 + 180 * ((density_norm[silver_mask] - 0.3) / 0.4))
    
    # Amber highlights
    amber_mask = density_norm > 0.7
    r_col[amber_mask] = np.maximum(r_col[amber_mask], 175 + 80 * ((density_norm[amber_mask] - 0.7) / 0.3))
    g_col[amber_mask] = np.maximum(g_col[amber_mask], 175 + 15 * ((density_norm[amber_mask] - 0.7) / 0.3))
    b_col[amber_mask] = np.maximum(b_col[amber_mask], 195 - 195 * ((density_norm[amber_mask] - 0.7) / 0.3))
