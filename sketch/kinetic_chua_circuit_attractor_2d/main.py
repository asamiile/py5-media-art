...
def step_chua(alpha, beta, m0, m1, dt=0.005):
    global x, y, z
    # Euler integration step for the Chua Circuit attractor
    ...
    mask = (np.abs(x_new) > 15) | (np.abs(y_new) > 15) | (np.abs(z_new) > 15) | np.isnan(x_new)
...
def draw():
    ...
    # Chua parameters with continuous modulation
    alpha = 15.6 + 0.5 * np.sin(t)
    beta = 28.0
    m0 = -1.143
    m1 = -0.714
    ...
    for _ in range(5):
        step_chua(alpha, beta, m0, m1, dt=0.002)
    ...
    # Map to screen
    # Chua typical size: x,y in [-3, 3], z in [-5, 5]
    ...
    eb_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[eb_mask] = np.maximum(r_col[eb_mask], 0 + 60 * ((density_norm[eb_mask] - 0.3) / 0.4))
    g_col[eb_mask] = np.maximum(g_col[eb_mask], 30 + 100 * ((density_norm[eb_mask] - 0.3) / 0.4))
    b_col[eb_mask] = np.maximum(b_col[eb_mask], 40 + 215 * ((density_norm[eb_mask] - 0.3) / 0.4))
    
    # Hot Pink highlights
    hp_mask = density_norm > 0.7
    r_col[hp_mask] = np.maximum(r_col[hp_mask], 60 + 195 * ((density_norm[hp_mask] - 0.7) / 0.3))
    g_col[hp_mask] = np.maximum(g_col[hp_mask], 130 + 0 * ((density_norm[hp_mask] - 0.7) / 0.3))
    b_col[hp_mask] = np.maximum(b_col[hp_mask], 255 - 120 * ((density_norm[hp_mask] - 0.7) / 0.3))
