...
def step_hadley(f_param, dt=0.01):
    global x, y, z
    # Euler integration step for the Hadley Circulation Attractor
    a = 0.2
    b = 4.0
    g = 1.0
...
def draw():
    ...
    f_param = 8.0 + 0.5 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_hadley(f_param, dt=0.005)
    ...
    # Map to screen
    # Hadley typical size: x in [-2, 10], y,z in [-4, 4]
    screen_x = (x_rot2 - 4.0) / 10.0 * SIZE[0]
    screen_y = (y_rot2) / 10.0 * SIZE[1]
    ...
    # Peach midtones
    peach_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[peach_mask] = np.maximum(r_col[peach_mask], 35 + 205 * ((density_norm[peach_mask] - 0.3) / 0.4))
    g_col[peach_mask] = np.maximum(g_col[peach_mask], 5 + 130 * ((density_norm[peach_mask] - 0.3) / 0.4))
    b_col[peach_mask] = np.maximum(b_col[peach_mask], 15 + 90 * ((density_norm[peach_mask] - 0.3) / 0.4))
    
    # Mint highlights
    mint_mask = density_norm > 0.7
    r_col[mint_mask] = np.maximum(r_col[mint_mask], 240 - 110 * ((density_norm[mint_mask] - 0.7) / 0.3))
    g_col[mint_mask] = np.maximum(g_col[mint_mask], 135 + 115 * ((density_norm[mint_mask] - 0.7) / 0.3))
    b_col[mint_mask] = np.maximum(b_col[mint_mask], 105 + 105 * ((density_norm[mint_mask] - 0.7) / 0.3))
