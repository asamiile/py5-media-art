...
def step_sprott_g(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case G Attractor
    dx = a * x + z
    dy = x * z - y
    dz = -x + y
...
def draw():
    ...
    # Map to screen
    # Sprott Case G typical size: x,y,z in [-5, 5]
    ...
    g_col[rose_mask] = np.maximum(g_col[rose_mask], 5 + 45 * ((density_norm[rose_mask] - 0.3) / 0.4))
    b_col[rose_mask] = np.maximum(b_col[rose_mask], 45 + 115 * ((density_norm[rose_mask] - 0.3) / 0.4))
    
    # Gold highlights
    gold_mask = density_norm > 0.7
    r_col[gold_mask] = np.maximum(r_col[gold_mask], 225 + 30 * ((density_norm[gold_mask] - 0.7) / 0.3))
    g_col[gold_mask] = np.maximum(g_col[gold_mask], 50 + 165 * ((density_norm[gold_mask] - 0.7) / 0.3))
    b_col[gold_mask] = np.maximum(b_col[gold_mask], 160 - 110 * ((density_norm[gold_mask] - 0.7) / 0.3))
