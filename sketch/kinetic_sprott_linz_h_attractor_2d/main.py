...
def step_sprott_h(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz H Attractor
    dx = -y + z * z
    dy = x + alpha * y
    dz = x - z
...
def draw():
    ...
    # Map to screen
    # Sprott H typical size: x,y,z in [-5, 5]
    ...
    r_col[vio_mask] = np.maximum(r_col[vio_mask], 30 + 130 * ((density_norm[vio_mask] - 0.3) / 0.4))
    g_col[vio_mask] = np.maximum(g_col[vio_mask], 30 + 35 * ((density_norm[vio_mask] - 0.3) / 0.4))
    b_col[vio_mask] = np.maximum(b_col[vio_mask], 35 + 175 * ((density_norm[vio_mask] - 0.3) / 0.4))
    
    # Rose highlights
    rose_mask = density_norm > 0.7
    r_col[rose_mask] = np.maximum(r_col[rose_mask], 160 + 95 * ((density_norm[rose_mask] - 0.7) / 0.3))
    g_col[rose_mask] = np.maximum(g_col[rose_mask], 65 + 65 * ((density_norm[rose_mask] - 0.7) / 0.3))
    b_col[rose_mask] = np.maximum(b_col[rose_mask], 210 + 20 * ((density_norm[rose_mask] - 0.7) / 0.3))
