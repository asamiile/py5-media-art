...
def step_arneodo(a, dt=0.005):
    global x, y, z
    b = 3.5
    c = -1.0
    
    # Euler integration step for the Arneodo Attractor
    dx = y
    dy = z
    dz = -a * x - b * y - z + c * (x ** 3)
...
def draw():
    ...
    # Map to screen
    # Arneodo typical size: x,y,z roughly in [-10, 10]
    ...
    r_col[peach_mask] = np.maximum(r_col[peach_mask], 35 + 220 * ((density_norm[peach_mask] - 0.3) / 0.4))
    g_col[peach_mask] = np.maximum(g_col[peach_mask], 15 + 185 * ((density_norm[peach_mask] - 0.3) / 0.4))
    b_col[peach_mask] = np.maximum(b_col[peach_mask], 65 + 85 * ((density_norm[peach_mask] - 0.3) / 0.4))
    
    # Mint highlights
    mint_mask = density_norm > 0.7
    r_col[mint_mask] = np.maximum(r_col[mint_mask], 255 - 105 * ((density_norm[mint_mask] - 0.7) / 0.3))
    g_col[mint_mask] = np.maximum(g_col[mint_mask], 200 + 55 * ((density_norm[mint_mask] - 0.7) / 0.3))
    b_col[mint_mask] = np.maximum(b_col[mint_mask], 150 + 105 * ((density_norm[mint_mask] - 0.7) / 0.3))
