...
def step_sprott_e(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case E Attractor
    dx = y * z
    dy = (x ** 2) - y
    dz = a - 4.0 * x
    ...
    mask = (np.abs(x_new) > 40) | (np.abs(y_new) > 40) | (np.abs(z_new) > 40) | np.isnan(x_new)
...
def draw():
    ...
    # Map to screen
    # Sprott Case E typical size: x,y,z in [-10, 10]
    ...
    g_col = 35 * (density_norm ** 1.5)
    b_col = 40 * (density_norm ** 1.5)
    
    # Coral midtones
    coral_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[coral_mask] = np.maximum(r_col[coral_mask], 5 + 225 * ((density_norm[coral_mask] - 0.3) / 0.4))
    g_col[coral_mask] = np.maximum(g_col[coral_mask], 35 + 85 * ((density_norm[coral_mask] - 0.3) / 0.4))
    b_col[coral_mask] = np.maximum(b_col[coral_mask], 40 + 40 * ((density_norm[coral_mask] - 0.3) / 0.4))
    
    # Cream highlights
    cream_mask = density_norm > 0.7
    r_col[cream_mask] = np.maximum(r_col[cream_mask], 230 + 25 * ((density_norm[cream_mask] - 0.7) / 0.3))
    g_col[cream_mask] = np.maximum(g_col[cream_mask], 120 + 130 * ((density_norm[cream_mask] - 0.7) / 0.3))
    b_col[cream_mask] = np.maximum(b_col[cream_mask], 80 + 150 * ((density_norm[cream_mask] - 0.7) / 0.3))
