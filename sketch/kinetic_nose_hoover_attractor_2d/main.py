...
def step_nose_hoover(a, dt=0.01):
    global x, y, z
    # Euler integration step for the Nosé-Hoover attractor
    dx = y
    dy = -x + y * z
    dz = a - y**2
...
def draw():
    ...
    # Nosé-Hoover parameter with continuous modulation
    a = 1.5 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_nose_hoover(a, dt=0.01)
    ...
    # Map to screen
    # Nosé-Hoover typical size: x,y in [-5, 5], z in [-5, 5]
    screen_x = (x_rot2 + 6.0) / 12.0 * SIZE[0]
    screen_y = (y_rot2 + 6.0) / 12.0 * SIZE[1]
    ...
    r_col[coral_mask] = np.maximum(r_col[coral_mask], 30 + 225 * ((density_norm[coral_mask] - 0.3) / 0.4))
    g_col[coral_mask] = np.maximum(g_col[coral_mask], 0 + 105 * ((density_norm[coral_mask] - 0.3) / 0.4))
    b_col[coral_mask] = np.maximum(b_col[coral_mask], 60 + 55 * ((density_norm[coral_mask] - 0.3) / 0.4))
    
    # Mango Yellow highlights
    mango_mask = density_norm > 0.7
    r_col[mango_mask] = np.maximum(r_col[mango_mask], 255 - 0 * ((density_norm[mango_mask] - 0.7) / 0.3))
    g_col[mango_mask] = np.maximum(g_col[mango_mask], 105 + 100 * ((density_norm[mango_mask] - 0.7) / 0.3))
    b_col[mango_mask] = np.maximum(b_col[mango_mask], 115 - 115 * ((density_norm[mango_mask] - 0.7) / 0.3))
