...
def step_rossler_hyperchaos(d, dt=0.01):
    global x, y, z, w
    a = 0.25
    b = 3.0
    c = 0.5
    
    # Euler integration step for the 4D Rössler Hyperchaos Attractor
    dx = -y - z
    dy = x + a * y + w
    dz = b + x * z
    dw = -c * z + d * w
    ...
    mask = (np.abs(x_new) > 40) | (np.abs(y_new) > 40) | (np.abs(z_new) > 40) | (np.abs(w_new) > 40) | np.isnan(x_new)
...
def draw():
    ...
    d = 0.05 + 0.03 * np.sin(t)
    
    # Run integration steps
    for _ in range(4):
        step_rossler_hyperchaos(d, dt=0.01)
        
    # Rotate 3D attractor gently over time
    theta = t * 0.4
    phi = np.sin(t * 1.5) * 0.4
    
    # We render the x, y, z projection of the 4D space
    # Rotate around Z
    ...
    # Map to screen
    # Rossler typical size: x,y in [-20, 20]
    ...
    # Sapphire midtones
    sapph_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[sapph_mask] = np.maximum(r_col[sapph_mask], 30 + 15 * ((density_norm[sapph_mask] - 0.3) / 0.4))
    g_col[sapph_mask] = np.maximum(g_col[sapph_mask], 15 + 85 * ((density_norm[sapph_mask] - 0.3) / 0.4))
    b_col[sapph_mask] = np.maximum(b_col[sapph_mask], 60 + 165 * ((density_norm[sapph_mask] - 0.3) / 0.4))
    
    # Silver highlights
    silv_mask = density_norm > 0.7
    r_col[silv_mask] = np.maximum(r_col[silv_mask], 45 + 175 * ((density_norm[silv_mask] - 0.7) / 0.3))
    g_col[silv_mask] = np.maximum(g_col[silv_mask], 100 + 120 * ((density_norm[silv_mask] - 0.7) / 0.3))
    b_col[silv_mask] = np.maximum(b_col[silv_mask], 225 + 30 * ((density_norm[silv_mask] - 0.7) / 0.3))
