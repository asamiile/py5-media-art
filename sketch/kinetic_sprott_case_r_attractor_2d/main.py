...
def step_sprott_r(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case R Attractor
    dx = a - y
    dy = 0.4 + z
    dz = x * y - z
...
def draw():
    ...
    # Map to screen
    # Sprott Case R typical size: x,y,z in [-5, 5]
    ...
    r_col[crimson_mask] = np.maximum(r_col[crimson_mask], 15 + 205 * ((density_norm[crimson_mask] - 0.3) / 0.4))
    g_col[crimson_mask] = np.maximum(g_col[crimson_mask], 15 + 5 * ((density_norm[crimson_mask] - 0.3) / 0.4))
    b_col[crimson_mask] = np.maximum(b_col[crimson_mask], 20 + 35 * ((density_norm[crimson_mask] - 0.3) / 0.4))
    
    # Cyan highlights
    cyan_mask = density_norm > 0.7
    r_col[cyan_mask] = np.maximum(r_col[cyan_mask], 220 - 185 * ((density_norm[cyan_mask] - 0.7) / 0.3))
    g_col[cyan_mask] = np.maximum(g_col[cyan_mask], 20 + 215 * ((density_norm[cyan_mask] - 0.7) / 0.3))
    b_col[cyan_mask] = np.maximum(b_col[cyan_mask], 55 + 180 * ((density_norm[cyan_mask] - 0.7) / 0.3))
