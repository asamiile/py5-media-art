...
def step_lu(a, b, c, dt=0.005):
    global x, y, z
    # Euler integration step for the Lü attractor
    dx = a * (y - x)
    dy = -x * z + c * y
    dz = x * y - b * z
...
def draw():
    ...
    # Lü parameter with continuous modulation
    a = 36.0
    b = 3.0
    c = 20.0 + 2.0 * np.sin(t)
    ...
    # Map to screen
    # Lü typical size: x,y in [-40, 40], z in [0, 50]
    ...
    # Copper midtones
    cop_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[cop_mask] = np.maximum(r_col[cop_mask], 60 + 120 * ((density_norm[cop_mask] - 0.3) / 0.4))
    g_col[cop_mask] = np.maximum(g_col[cop_mask], 0 + 80 * ((density_norm[cop_mask] - 0.3) / 0.4))
    b_col[cop_mask] = np.maximum(b_col[cop_mask], 20 + 20 * ((density_norm[cop_mask] - 0.3) / 0.4))
    
    # Rose Gold highlights
    rg_mask = density_norm > 0.7
    r_col[rg_mask] = np.maximum(r_col[rg_mask], 180 + 75 * ((density_norm[rg_mask] - 0.7) / 0.3))
    g_col[rg_mask] = np.maximum(g_col[rg_mask], 80 + 90 * ((density_norm[rg_mask] - 0.7) / 0.3))
    b_col[rg_mask] = np.maximum(b_col[rg_mask], 40 + 130 * ((density_norm[rg_mask] - 0.7) / 0.3))
