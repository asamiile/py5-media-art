...
def step_sprott_j(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case J Attractor
    dx = a * z
    dy = -2.0 * y + z
    dz = -x + y + (y ** 2)
...
def draw():
    ...
    # Map to screen
    # Sprott Case J typical size: x,y,z in [-5, 5]
    ...
    r_col[emerald_mask] = np.maximum(r_col[emerald_mask], 5 + 45 * ((density_norm[emerald_mask] - 0.3) / 0.4))
    g_col[emerald_mask] = np.maximum(g_col[emerald_mask], 15 + 185 * ((density_norm[emerald_mask] - 0.3) / 0.4))
    b_col[emerald_mask] = np.maximum(b_col[emerald_mask], 50 + 60 * ((density_norm[emerald_mask] - 0.3) / 0.4))
    
    # Topaz highlights
    topaz_mask = density_norm > 0.7
    r_col[topaz_mask] = np.maximum(r_col[topaz_mask], 50 + 205 * ((density_norm[topaz_mask] - 0.7) / 0.3))
    g_col[topaz_mask] = np.maximum(g_col[topaz_mask], 200 + 45 * ((density_norm[topaz_mask] - 0.7) / 0.3))
    b_col[topaz_mask] = np.maximum(b_col[topaz_mask], 110 + 20 * ((density_norm[topaz_mask] - 0.7) / 0.3))
