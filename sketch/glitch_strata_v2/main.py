...
def draw():
    ...
    # Palette: Obsidian & Gold (Luxury Glitch)
    colors = [
        [10, 10, 12],    # Obsidian
        [180, 140, 40],  # Deep Gold
        [255, 215, 0],   # Bright Gold
        [160, 165, 170], # Steel/Silver
        [255, 240, 200], # Pale Amber
        [40, 40, 45]     # Dark Shadow
    ]
    ...
    for i in range(len(y_coords) - 1):
        ...
        if style < 0.4:
            ...
            for y in range(y_start, y_end):
                ...
                row_noise = np.random.normal(0, 3, (w_px, 3))
            ...
            for x in range(0, w_px, block_w):
                if py5.random(1) > 0.3:
                    ...
                    # Randomly corrupt block color with bright gold or silver
                    if py5.random(1) > 0.8:
                        col = np.array([255, 255, 255]) if py5.random(1) > 0.5 else np.array([255, 215, 0])
            ...
            for y in range(y_start, y_end):
                ...
                col = c1 if py5.random(1) > 0.2 else np.array([255, 215, 0])
    ...
    for _ in range(15):
        ...
        spike_w = int(py5.random(1, w_px // 300))
        col = np.array(colors[int(py5.random(1, len(colors)))])
        actual_spike_w = min(spike_w, w_px - x)
        py5.np_pixels[:, x:x+actual_spike_w, :3] = col.astype(np.uint8)
        
    # Scanline overlay
    for y in range(0, h_px, 3):
        py5.np_pixels[y, :, :3] = (py5.np_pixels[y, :, :3] * 0.85).astype(np.uint8)
    ...
    py5.save_frame(str(OUTPUT_DIR / "glitch_strata_v2_p1.png"))
    print(f"Saved preview to {OUTPUT_DIR / 'glitch_strata_v2_p1.png'}")
    py5.exit_sketch()
