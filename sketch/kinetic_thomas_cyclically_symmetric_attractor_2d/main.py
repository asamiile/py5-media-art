...
def step_thomas(b, dt=0.05):
    global x, y, z
    # Euler integration step for the Thomas Cyclically Symmetric Attractor
    dx = np.sin(y) - b * x
    dy = np.sin(z) - b * y
    dz = np.sin(x) - b * z
    ...
    x_new[mask] = np.random.uniform(-4, 4, np.sum(mask)).astype(np.float32)
    y_new[mask] = np.random.uniform(-4, 4, np.sum(mask)).astype(np.float32)
    z_new[mask] = np.random.uniform(-4, 4, np.sum(mask)).astype(np.float32)
...
def draw():
    ...
    # Thomas parameter with continuous modulation
    b = 0.19 + 0.01 * np.sin(t)
    ...
    # Map to screen
    # Thomas typical size: x,y,z in [-5, 5]
    ...
    r_col[mint_mask] = np.maximum(r_col[mint_mask], 0 + 100 * ((density_norm[mint_mask] - 0.3) / 0.4))
    g_col[mint_mask] = np.maximum(g_col[mint_mask], 50 + 205 * ((density_norm[mint_mask] - 0.3) / 0.4))
    b_col[mint_mask] = np.maximum(b_col[mint_mask], 60 + 100 * ((density_norm[mint_mask] - 0.3) / 0.4))
    
    # Soft Gold highlights
    gold_mask = density_norm > 0.7
    r_col[gold_mask] = np.maximum(r_col[gold_mask], 100 + 155 * ((density_norm[gold_mask] - 0.7) / 0.3))
    g_col[gold_mask] = np.maximum(g_col[gold_mask], 255 - 40 * ((density_norm[gold_mask] - 0.7) / 0.3))
    b_col[gold_mask] = np.maximum(b_col[gold_mask], 160 - 60 * ((density_norm[gold_mask] - 0.7) / 0.3))
