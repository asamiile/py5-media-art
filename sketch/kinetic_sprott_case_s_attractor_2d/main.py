...
def step_sprott_s(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case S Attractor
    dx = -x - a * y
    dy = x + (z ** 2)
    dz = 1.0 + x
...
def draw():
    ...
    # Map to screen
    # Sprott Case S typical size: x,y,z in [-5, 5]
    ...
    r_col[violet_mask] = np.maximum(r_col[violet_mask], 35 + 105 * ((density_norm[violet_mask] - 0.3) / 0.4))
    g_col[violet_mask] = np.maximum(g_col[violet_mask], 25 + 25 * ((density_norm[violet_mask] - 0.3) / 0.4))
    b_col[violet_mask] = np.maximum(b_col[violet_mask], 15 + 175 * ((density_norm[violet_mask] - 0.3) / 0.4))
    
    # Lime highlights
    lime_mask = density_norm > 0.7
    r_col[lime_mask] = np.maximum(r_col[lime_mask], 140 - 50 * ((density_norm[lime_mask] - 0.7) / 0.3))
    g_col[lime_mask] = np.maximum(g_col[lime_mask], 50 + 175 * ((density_norm[lime_mask] - 0.7) / 0.3))
    b_col[lime_mask] = np.maximum(b_col[lime_mask], 190 - 130 * ((density_norm[lime_mask] - 0.7) / 0.3))
