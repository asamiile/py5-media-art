...
def step_wei(b, dt=0.005):
    ...
    # Euler integration step for the Wei Attractor
    dx = a * (y - x)
    dy = b * x + c * y - x * z
    dz = d * z + x * y
...
def draw():
    ...
    # Map to screen
    # Wei typical size: x,y,z in [-100, 100]
    ...
    r_col[peach_mask] = np.maximum(r_col[peach_mask], 40 + 215 * ((density_norm[peach_mask] - 0.3) / 0.4))
    g_col[peach_mask] = np.maximum(g_col[peach_mask], 15 + 130 * ((density_norm[peach_mask] - 0.3) / 0.4))
    b_col[peach_mask] = np.maximum(b_col[peach_mask], 25 + 115 * ((density_norm[peach_mask] - 0.3) / 0.4))
    
    # Cream highlights
    cream_mask = density_norm > 0.7
    r_col[cream_mask] = np.maximum(r_col[cream_mask], 255)
    g_col[cream_mask] = np.maximum(g_col[cream_mask], 145 + 110 * ((density_norm[cream_mask] - 0.7) / 0.3))
    b_col[cream_mask] = np.maximum(b_col[cream_mask], 140 + 105 * ((density_norm[cream_mask] - 0.7) / 0.3))
