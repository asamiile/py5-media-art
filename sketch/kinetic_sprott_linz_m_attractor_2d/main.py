...
def step_sprott_m(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz M Attractor
    dx = -z
    dy = -x * x - y
    dz = alpha + alpha * x + y
...
def draw():
    ...
    alpha = 1.7 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_m(alpha, dt=0.005)
    ...
    # Map to screen
    # Sprott M typical size: x,y,z in [-5, 5]
    ...
    g_col[coral_mask] = np.maximum(g_col[coral_mask], 20 + 95 * ((density_norm[coral_mask] - 0.3) / 0.4))
    b_col[coral_mask] = np.maximum(b_col[coral_mask], 55 + 25 * ((density_norm[coral_mask] - 0.3) / 0.4))
    
    # Tangerine highlights
    tang_mask = density_norm > 0.7
    r_col[tang_mask] = np.maximum(r_col[tang_mask], 255 - 0 * ((density_norm[tang_mask] - 0.7) / 0.3))
    g_col[tang_mask] = np.maximum(g_col[tang_mask], 115 + 85 * ((density_norm[tang_mask] - 0.7) / 0.3))
    b_col[tang_mask] = np.maximum(b_col[tang_mask], 80 - 60 * ((density_norm[tang_mask] - 0.7) / 0.3))
