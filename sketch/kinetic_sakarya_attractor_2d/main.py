...
def step_sakarya(a, dt=0.01):
    global x, y, z
    # Euler integration step for the Sakarya Attractor
    b = 0.3
...
def draw():
    ...
    # Sakarya parameter with continuous modulation
    a = 0.4 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sakarya(a, dt=0.005)
    ...
    # Map to screen
    # Sakarya typical size: x,y,z in [-5, 5]
    ...
    r_col[teal_mask] = np.maximum(r_col[teal_mask], 0 + 0 * ((density_norm[teal_mask] - 0.3) / 0.4))
    g_col[teal_mask] = np.maximum(g_col[teal_mask], 15 + 113 * ((density_norm[teal_mask] - 0.3) / 0.4))
    b_col[teal_mask] = np.maximum(b_col[teal_mask], 40 + 88 * ((density_norm[teal_mask] - 0.3) / 0.4))
    
    # Aquamarine highlights
    aqua_mask = density_norm > 0.7
    r_col[aqua_mask] = np.maximum(r_col[aqua_mask], 0 + 127 * ((density_norm[aqua_mask] - 0.7) / 0.3))
    g_col[aqua_mask] = np.maximum(g_col[aqua_mask], 128 + 127 * ((density_norm[aqua_mask] - 0.7) / 0.3))
    b_col[aqua_mask] = np.maximum(b_col[aqua_mask], 128 + 84 * ((density_norm[aqua_mask] - 0.7) / 0.3))
