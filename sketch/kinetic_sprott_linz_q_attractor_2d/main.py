...
def step_sprott_q(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz Q Attractor
    dx = -z
    dy = x - y
    dz = alpha * x + y * y + z
...
def draw():
    ...
    alpha = 3.1 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_q(alpha, dt=0.005)
    ...
    # Map to screen
    # Sprott Q typical size: x,y,z in [-5, 5]
    ...
    # Crimson base
    r_col = 45 * (density_norm ** 1.5)
    g_col = 15 * (density_norm ** 1.5)
    b_col = 20 * (density_norm ** 1.5)
    
    # Tangerine midtones
    tang_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[tang_mask] = np.maximum(r_col[tang_mask], 45 + 185 * ((density_norm[tang_mask] - 0.3) / 0.4))
    g_col[tang_mask] = np.maximum(g_col[tang_mask], 15 + 105 * ((density_norm[tang_mask] - 0.3) / 0.4))
    b_col[tang_mask] = np.maximum(b_col[tang_mask], 20 + 20 * ((density_norm[tang_mask] - 0.3) / 0.4))
    
    # Amber highlights
    amb_mask = density_norm > 0.7
    r_col[amb_mask] = np.maximum(r_col[amb_mask], 230 + 25 * ((density_norm[amb_mask] - 0.7) / 0.3))
    g_col[amb_mask] = np.maximum(g_col[amb_mask], 120 + 95 * ((density_norm[amb_mask] - 0.7) / 0.3))
    b_col[amb_mask] = np.maximum(b_col[amb_mask], 40 + 60 * ((density_norm[amb_mask] - 0.7) / 0.3))
