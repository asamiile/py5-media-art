...
def step_dequan_li(a, dt=0.002):
    ...
    # Euler integration step for the Dequan Li Attractor
    dx = a * (y - x) + c_param * x * z
    dy = k * x + f * y - x * z
    dz = m * z + x * y - e * x * x
    ...
    # Keep particles bounded (Dequan Li attractor typically occupies range [-200, 200])
    mask = (np.abs(x_new) > 400) | (np.abs(y_new) > 400) | (np.abs(z_new) > 400) | np.isnan(x_new)
...
def draw():
    ...
    # Map to screen
    # Dequan Li typical size: x,y,z in [-150, 150] roughly
    screen_x = (x_rot2 + 150.0) / 300.0 * SIZE[0]
    screen_y = (y_rot2 + 150.0) / 300.0 * SIZE[1]
    ...
    # Magenta base
    r_col = 55 * (density_norm ** 1.5)
    g_col = 15 * (density_norm ** 1.5)
    b_col = 55 * (density_norm ** 1.5)
    
    # Cyan midtones
    cyan_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[cyan_mask] = np.maximum(r_col[cyan_mask], 55 - 25 * ((density_norm[cyan_mask] - 0.3) / 0.4))
    g_col[cyan_mask] = np.maximum(g_col[cyan_mask], 15 + 235 * ((density_norm[cyan_mask] - 0.3) / 0.4))
    b_col[cyan_mask] = np.maximum(b_col[cyan_mask], 55 + 200 * ((density_norm[cyan_mask] - 0.3) / 0.4))
    
    # White highlights
    white_mask = density_norm > 0.7
    r_col[white_mask] = np.maximum(r_col[white_mask], 30 + 225 * ((density_norm[white_mask] - 0.7) / 0.3))
    g_col[white_mask] = np.maximum(g_col[white_mask], 250 + 5 * ((density_norm[white_mask] - 0.7) / 0.3))
    b_col[white_mask] = np.maximum(b_col[white_mask], 255)
