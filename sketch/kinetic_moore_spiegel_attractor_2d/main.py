...
def step_moore_spiegel(r_param, dt=0.005):
    global x, y, z
    # Euler integration step for the Moore-Spiegel Attractor
    t_param = 100.0
    
    dx = y
    dy = z
    dz = -z - (t_param - r_param + r_param * x * x) * y - t_param * x
    ...
    mask = (np.abs(x_new) > 50) | (np.abs(y_new) > 50) | (np.abs(z_new) > 50) | np.isnan(x_new)
...
def draw():
    ...
    r_param = 100.0 + 5.0 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_moore_spiegel(r_param, dt=0.002)
    ...
    # Map to screen
    # Moore-Spiegel typical size: x in [-3, 3], y in [-15, 15]
    ...
    # Amber midtones
    amb_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[amb_mask] = np.maximum(r_col[amb_mask], 20 + 215 * ((density_norm[amb_mask] - 0.3) / 0.4))
    g_col[amb_mask] = np.maximum(g_col[amb_mask], 25 + 95 * ((density_norm[amb_mask] - 0.3) / 0.4))
    b_col[amb_mask] = np.maximum(b_col[amb_mask], 35 + 5 * ((density_norm[amb_mask] - 0.3) / 0.4))
    
    # Gold highlights
    gold_mask = density_norm > 0.7
    r_col[gold_mask] = np.maximum(r_col[gold_mask], 235 + 20 * ((density_norm[gold_mask] - 0.7) / 0.3))
    g_col[gold_mask] = np.maximum(g_col[gold_mask], 120 + 105 * ((density_norm[gold_mask] - 0.7) / 0.3))
    b_col[gold_mask] = np.maximum(b_col[gold_mask], 40 + 65 * ((density_norm[gold_mask] - 0.7) / 0.3))
