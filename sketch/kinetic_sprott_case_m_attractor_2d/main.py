...
def step_sprott_m(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case M Attractor
    dx = -z
    dy = -(x ** 2) - y
    dz = 1.7 + a * x + y
...
def draw():
    ...
    # Map to screen
    # Sprott Case M typical size: x,y,z in [-5, 5]
    ...
    # Magenta midtones
    magenta_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[magenta_mask] = np.maximum(r_col[magenta_mask], 15 + 205 * ((density_norm[magenta_mask] - 0.3) / 0.4))
    g_col[magenta_mask] = np.maximum(g_col[magenta_mask], 5 + 30 * ((density_norm[magenta_mask] - 0.3) / 0.4))
    b_col[magenta_mask] = np.maximum(b_col[magenta_mask], 45 + 140 * ((density_norm[magenta_mask] - 0.3) / 0.4))
    
    # Teal highlights
    teal_mask = density_norm > 0.7
    r_col[teal_mask] = np.maximum(r_col[teal_mask], 220 - 180 * ((density_norm[teal_mask] - 0.7) / 0.3))
    g_col[teal_mask] = np.maximum(g_col[teal_mask], 35 + 195 * ((density_norm[teal_mask] - 0.7) / 0.3))
    b_col[teal_mask] = np.maximum(b_col[teal_mask], 185 + 50 * ((density_norm[teal_mask] - 0.7) / 0.3))
