...
def step_sprott_r(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz R Attractor
    dx = alpha - y
    dy = 0.4 + z
    dz = x * y - z
...
def draw():
    ...
    alpha = 0.9 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_r(alpha, dt=0.005)
    ...
    # Map to screen
    # Sprott R typical size: x,y,z in [-5, 5]
    ...
    r_col[mag_mask] = np.maximum(r_col[mag_mask], 15 + 185 * ((density_norm[mag_mask] - 0.3) / 0.4))
    g_col[mag_mask] = np.maximum(g_col[mag_mask], 55 - 45 * ((density_norm[mag_mask] - 0.3) / 0.4))
    b_col[mag_mask] = np.maximum(b_col[mag_mask], 65 + 135 * ((density_norm[mag_mask] - 0.3) / 0.4))
    
    # White highlights
    wh_mask = density_norm > 0.7
    r_col[wh_mask] = np.maximum(r_col[wh_mask], 200 + 55 * ((density_norm[wh_mask] - 0.7) / 0.3))
    g_col[wh_mask] = np.maximum(g_col[wh_mask], 10 + 245 * ((density_norm[wh_mask] - 0.7) / 0.3))
    b_col[wh_mask] = np.maximum(b_col[wh_mask], 200 + 55 * ((density_norm[wh_mask] - 0.7) / 0.3))
