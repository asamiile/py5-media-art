...
def step_sprott_s(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz S Attractor
    dx = -x - alpha * y
    dy = x + z * z
    dz = 1 + x
...
def draw():
    ...
    alpha = 4.0 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_s(alpha, dt=0.005)
    ...
    # Map to screen
    # Sprott S typical size: x,y,z in [-5, 5]
    ...
    b_col = 85 * (density_norm ** 1.5)
    
    # Purple midtones
    purp_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[purp_mask] = np.maximum(r_col[purp_mask], 15 + 135 * ((density_norm[purp_mask] - 0.3) / 0.4))
    g_col[purp_mask] = np.maximum(g_col[purp_mask], 15 + 35 * ((density_norm[purp_mask] - 0.3) / 0.4))
    b_col[purp_mask] = np.maximum(b_col[purp_mask], 85 + 115 * ((density_norm[purp_mask] - 0.3) / 0.4))
    
    # Hot Pink highlights
    pink_mask = density_norm > 0.7
    r_col[pink_mask] = np.maximum(r_col[pink_mask], 150 + 105 * ((density_norm[pink_mask] - 0.7) / 0.3))
    g_col[pink_mask] = np.maximum(g_col[pink_mask], 50 + 55 * ((density_norm[pink_mask] - 0.7) / 0.3))
    b_col[pink_mask] = np.maximum(b_col[pink_mask], 200 + 10 * ((density_norm[pink_mask] - 0.7) / 0.3))
