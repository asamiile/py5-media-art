...
def step_sprott_n(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz N Attractor
    dx = -alpha * y
    dy = x + z * z
    dz = 1.0 + y - alpha * z
...
def draw():
    ...
    # Map to screen
    # Sprott N typical size: x,y,z in [-5, 5]
    ...
    g_col = 22 * (density_norm ** 1.5)
    b_col = 25 * (density_norm ** 1.5)
    
    # Violet midtones
    viol_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[viol_mask] = np.maximum(r_col[viol_mask], 20 + 90 * ((density_norm[viol_mask] - 0.3) / 0.4))
    g_col[viol_mask] = np.maximum(g_col[viol_mask], 22 + 40 * ((density_norm[viol_mask] - 0.3) / 0.4))
    b_col[viol_mask] = np.maximum(b_col[viol_mask], 25 + 130 * ((density_norm[viol_mask] - 0.3) / 0.4))
    
    # Rose highlights
    rose_mask = density_norm > 0.7
    r_col[rose_mask] = np.maximum(r_col[rose_mask], 110 + 130 * ((density_norm[rose_mask] - 0.7) / 0.3))
    g_col[rose_mask] = np.maximum(g_col[rose_mask], 62 + 90 * ((density_norm[rose_mask] - 0.7) / 0.3))
    b_col[rose_mask] = np.maximum(b_col[rose_mask], 155 + 50 * ((density_norm[rose_mask] - 0.7) / 0.3))
