...
def step_sprott_n(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case N Attractor
    dx = -a * y
    dy = x + (z ** 2)
    dz = 1.0 + y - 2.0 * z
...
def draw():
    ...
    # Map to screen
    # Sprott Case N typical size: x,y,z in [-5, 5]
    ...
    g_col = 33 * (density_norm ** 1.5)
    b_col = 30 * (density_norm ** 1.5)
    
    # Mustard midtones
    mustard_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[mustard_mask] = np.maximum(r_col[mustard_mask], 35 + 200 * ((density_norm[mustard_mask] - 0.3) / 0.4))
    g_col[mustard_mask] = np.maximum(g_col[mustard_mask], 33 + 175 * ((density_norm[mustard_mask] - 0.3) / 0.4))
    b_col[mustard_mask] = np.maximum(b_col[mustard_mask], 30 + 35 * ((density_norm[mustard_mask] - 0.3) / 0.4))
    
    # Rust highlights
    rust_mask = density_norm > 0.7
    r_col[rust_mask] = np.maximum(r_col[rust_mask], 235 - 70 * ((density_norm[rust_mask] - 0.7) / 0.3))
    g_col[rust_mask] = np.maximum(g_col[rust_mask], 208 - 140 * ((density_norm[rust_mask] - 0.7) / 0.3))
    b_col[rust_mask] = np.maximum(b_col[rust_mask], 65 - 35 * ((density_norm[rust_mask] - 0.7) / 0.3))
