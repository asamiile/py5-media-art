...
def step_hyperchaotic_chen(r, dt=0.002):
    global x, y, z, w
    ...
    # Euler integration step for the Hyperchaotic Chen Attractor
    dx = a * (y - x) + w
    dy = d * x - x * z + c * y
    dz = x * y - b * z
    dw = y * z + r * w
...
def draw():
    ...
    # Run integration steps (smaller dt for Chen)
    for _ in range(4):
        step_hyperchaotic_chen(r, dt=0.002)
    ...
    # Map to screen
    # Hyperchaotic Chen typical size: x,y in [-40, 40], z in [0, 60]
    ...
    # Chartreuse midtones
    chartreuse_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[chartreuse_mask] = np.maximum(r_col[chartreuse_mask], 5 + 185 * ((density_norm[chartreuse_mask] - 0.3) / 0.4))
    g_col[chartreuse_mask] = np.maximum(g_col[chartreuse_mask], 45 + 210 * ((density_norm[chartreuse_mask] - 0.3) / 0.4))
    b_col[chartreuse_mask] = np.maximum(b_col[chartreuse_mask], 25 - 25 * ((density_norm[chartreuse_mask] - 0.3) / 0.4))
    
    # Silver highlights
    silver_mask = density_norm > 0.7
    r_col[silver_mask] = np.maximum(r_col[silver_mask], 190 + 65 * ((density_norm[silver_mask] - 0.7) / 0.3))
    g_col[silver_mask] = np.maximum(g_col[silver_mask], 255)
    b_col[silver_mask] = np.maximum(b_col[silver_mask], 190 + 65 * ((density_norm[silver_mask] - 0.7) / 0.3))
