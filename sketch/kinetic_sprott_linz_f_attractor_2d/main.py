...
def step_sprott_f(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz F Attractor
    dx = y + z
    dy = -x + alpha * y
    dz = x * x - z
...
def draw():
    ...
    alpha = 0.5 + 0.05 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_f(alpha, dt=0.005)
    ...
    r_col[teal_mask] = np.maximum(r_col[teal_mask], 15 + 20 * ((density_norm[teal_mask] - 0.3) / 0.4))
    g_col[teal_mask] = np.maximum(g_col[teal_mask], 5 + 160 * ((density_norm[teal_mask] - 0.3) / 0.4))
    b_col[teal_mask] = np.maximum(b_col[teal_mask], 35 + 175 * ((density_norm[teal_mask] - 0.3) / 0.4))
    
    # Silver highlights
    silv_mask = density_norm > 0.7
    r_col[silv_mask] = np.maximum(r_col[silv_mask], 35 + 195 * ((density_norm[silv_mask] - 0.7) / 0.3))
    g_col[silv_mask] = np.maximum(g_col[silv_mask], 165 + 75 * ((density_norm[silv_mask] - 0.7) / 0.3))
    b_col[silv_mask] = np.maximum(b_col[silv_mask], 210 + 40 * ((density_norm[silv_mask] - 0.7) / 0.3))
