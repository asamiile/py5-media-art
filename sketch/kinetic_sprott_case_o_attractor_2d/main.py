...
def step_sprott_o(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case O Attractor
    dx = y
    dy = x - z
    dz = x + x * z + a * y
...
def draw():
    ...
    # Map to screen
    # Sprott Case O typical size: x,y,z in [-5, 5]
    ...
    g_col = 38 * (density_norm ** 1.5)
    b_col = 45 * (density_norm ** 1.5)
    
    # Plum midtones
    plum_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[plum_mask] = np.maximum(r_col[plum_mask], 35 + 115 * ((density_norm[plum_mask] - 0.3) / 0.4))
    g_col[plum_mask] = np.maximum(g_col[plum_mask], 38 + 25 * ((density_norm[plum_mask] - 0.3) / 0.4))
    b_col[plum_mask] = np.maximum(b_col[plum_mask], 45 + 120 * ((density_norm[plum_mask] - 0.3) / 0.4))
    
    # Coral highlights
    coral_mask = density_norm > 0.7
    r_col[coral_mask] = np.maximum(r_col[coral_mask], 150 + 105 * ((density_norm[coral_mask] - 0.7) / 0.3))
    g_col[coral_mask] = np.maximum(g_col[coral_mask], 63 + 65 * ((density_norm[coral_mask] - 0.7) / 0.3))
    b_col[coral_mask] = np.maximum(b_col[coral_mask], 165 - 115 * ((density_norm[coral_mask] - 0.7) / 0.3))
