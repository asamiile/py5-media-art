...
def step_sprott_q(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case Q Attractor
    dx = -z
    dy = x - y
    dz = a * x + (y ** 2) + z
...
def draw():
    ...
    # Map to screen
    # Sprott Case Q typical size: x,y,z in [-5, 5]
    ...
    # Ocean midtones
    ocean_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[ocean_mask] = np.maximum(r_col[ocean_mask], 35 - 15 * ((density_norm[ocean_mask] - 0.3) / 0.4))
    g_col[ocean_mask] = np.maximum(g_col[ocean_mask], 45 + 95 * ((density_norm[ocean_mask] - 0.3) / 0.4))
    b_col[ocean_mask] = np.maximum(b_col[ocean_mask], 55 + 160 * ((density_norm[ocean_mask] - 0.3) / 0.4))
    
    # Amber highlights
    amber_mask = density_norm > 0.7
    r_col[amber_mask] = np.maximum(r_col[amber_mask], 20 + 235 * ((density_norm[amber_mask] - 0.7) / 0.3))
    g_col[amber_mask] = np.maximum(g_col[amber_mask], 140 + 55 * ((density_norm[amber_mask] - 0.7) / 0.3))
    b_col[amber_mask] = np.maximum(b_col[amber_mask], 215 - 195 * ((density_norm[amber_mask] - 0.7) / 0.3))
