...
def step_sprott_l(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case L Attractor
    dx = y + a * z
    dy = 0.9 * (x ** 2) - y
    dz = 1.0 - x
...
def draw():
    ...
    # Map to screen
    # Sprott Case L typical size: x,y,z in [-8, 8]
    screen_x = (x_rot2 + 8.0) / 16.0 * SIZE[0]
    screen_y = (y_rot2 + 8.0) / 16.0 * SIZE[1]
    ...
    r_col[rose_mask] = np.maximum(r_col[rose_mask], 15 + 230 * ((density_norm[rose_mask] - 0.3) / 0.4))
    g_col[rose_mask] = np.maximum(g_col[rose_mask], 15 + 40 * ((density_norm[rose_mask] - 0.3) / 0.4))
    b_col[rose_mask] = np.maximum(b_col[rose_mask], 15 + 80 * ((density_norm[rose_mask] - 0.3) / 0.4))
    
    # Gold highlights
    gold_mask = density_norm > 0.7
    r_col[gold_mask] = np.maximum(r_col[gold_mask], 245 + 10 * ((density_norm[gold_mask] - 0.7) / 0.3))
    g_col[gold_mask] = np.maximum(g_col[gold_mask], 55 + 160 * ((density_norm[gold_mask] - 0.7) / 0.3))
    b_col[gold_mask] = np.maximum(b_col[gold_mask], 95 - 65 * ((density_norm[gold_mask] - 0.7) / 0.3))
