...
def step_wang_sun(d, dt=0.01):
    ...
    # Euler integration step for the Wang-Sun Attractor
    dx = a * x + c * y * z
    dy = b * x + d * y - x * z
    dz = e * z + f * x * y
...
def draw():
    ...
    # Map to screen
    # Wang-Sun typical size: x,y,z roughly in [-2, 2]
    ...
    # Crimson midtones
    crim_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[crim_mask] = np.maximum(r_col[crim_mask], 20 + 200 * ((density_norm[crim_mask] - 0.3) / 0.4))
    g_col[crim_mask] = np.maximum(g_col[crim_mask], 15 + 15 * ((density_norm[crim_mask] - 0.3) / 0.4))
    b_col[crim_mask] = np.maximum(b_col[crim_mask], 25 + 30 * ((density_norm[crim_mask] - 0.3) / 0.4))
    
    # Pearl highlights
    pearl_mask = density_norm > 0.7
    r_col[pearl_mask] = np.maximum(r_col[pearl_mask], 220 + 35 * ((density_norm[pearl_mask] - 0.7) / 0.3))
    g_col[pearl_mask] = np.maximum(g_col[pearl_mask], 30 + 220 * ((density_norm[pearl_mask] - 0.7) / 0.3))
    b_col[pearl_mask] = np.maximum(b_col[pearl_mask], 55 + 200 * ((density_norm[pearl_mask] - 0.7) / 0.3))
