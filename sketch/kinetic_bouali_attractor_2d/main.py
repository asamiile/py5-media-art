...
def step_bouali(a, dt=0.005):
    global x, y, z
    alpha = 0.3
    beta = 1.5
    gamma = 0.05
    
    # Euler integration step for the Bouali Attractor
    dx = x * (a - y) + alpha * z
    dy = -y * (1.0 - x * x)
    dz = -x * (beta - z) - gamma * z
...
def draw():
    ...
    # Map to screen
    # Bouali typical size: x,y,z in [-5, 5] roughly
    ...
    # Ruby base
    r_col = 65 * (density_norm ** 1.5)
    g_col = 15 * (density_norm ** 1.5)
    b_col = 35 * (density_norm ** 1.5)
    
    # Gold midtones
    gold_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[gold_mask] = np.maximum(r_col[gold_mask], 65 + 190 * ((density_norm[gold_mask] - 0.3) / 0.4))
    g_col[gold_mask] = np.maximum(g_col[gold_mask], 15 + 185 * ((density_norm[gold_mask] - 0.3) / 0.4))
    b_col[gold_mask] = np.maximum(b_col[gold_mask], 35 - 20 * ((density_norm[gold_mask] - 0.3) / 0.4))
    
    # White highlights
    white_mask = density_norm > 0.7
    r_col[white_mask] = np.maximum(r_col[white_mask], 255)
    g_col[white_mask] = np.maximum(g_col[white_mask], 200 + 55 * ((density_norm[white_mask] - 0.7) / 0.3))
    b_col[white_mask] = np.maximum(b_col[white_mask], 15 + 240 * ((density_norm[white_mask] - 0.7) / 0.3))
