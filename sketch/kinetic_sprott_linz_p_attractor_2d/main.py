...
def step_sprott_p(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz P Attractor
    dx = alpha * y + z
    dy = -x + y * y
    dz = x + y
...
def draw():
    ...
    alpha = 2.7 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_p(alpha, dt=0.005)
    ...
    # Map to screen
    # Sprott P typical size: x,y,z in [-5, 5]
    ...
    g_col = 55 * (density_norm ** 1.5)
    b_col = 25 * (density_norm ** 1.5)
    
    # Olive midtones
    olive_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[olive_mask] = np.maximum(r_col[olive_mask], 15 + 115 * ((density_norm[olive_mask] - 0.3) / 0.4))
    g_col[olive_mask] = np.maximum(g_col[olive_mask], 55 + 75 * ((density_norm[olive_mask] - 0.3) / 0.4))
    b_col[olive_mask] = np.maximum(b_col[olive_mask], 25 + 15 * ((density_norm[olive_mask] - 0.3) / 0.4))
    
    # Chartreuse highlights
    chart_mask = density_norm > 0.7
    r_col[chart_mask] = np.maximum(r_col[chart_mask], 130 + 90 * ((density_norm[chart_mask] - 0.7) / 0.3))
    g_col[chart_mask] = np.maximum(g_col[chart_mask], 130 + 125 * ((density_norm[chart_mask] - 0.7) / 0.3))
    b_col[chart_mask] = np.maximum(b_col[chart_mask], 40 - 40 * ((density_norm[chart_mask] - 0.7) / 0.3))
