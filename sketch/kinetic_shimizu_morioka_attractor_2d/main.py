...
def step_shimizu_morioka(a, dt=0.01):
    global x, y, z
    l = 0.85
    
    # Euler integration step for the Shimizu-Morioka Attractor
    dx = y
    dy = x - l * y - x * z
    dz = -a * z + x * x
...
def draw():
    ...
    a = 0.375 + 0.05 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_shimizu_morioka(a, dt=0.005)
    ...
    # Map to screen
    # Shimizu-Morioka typical size: x,y,z in [-2, 2] roughly
    ...
    r_col[ruby_mask] = np.maximum(r_col[ruby_mask], 35 + 175 * ((density_norm[ruby_mask] - 0.3) / 0.4))
    g_col[ruby_mask] = np.maximum(g_col[ruby_mask], 35 - 20 * ((density_norm[ruby_mask] - 0.3) / 0.4))
    b_col[ruby_mask] = np.maximum(b_col[ruby_mask], 40 - 20 * ((density_norm[ruby_mask] - 0.3) / 0.4))
    
    # Pearl highlights
    pearl_mask = density_norm > 0.7
    r_col[pearl_mask] = np.maximum(r_col[pearl_mask], 210 + 35 * ((density_norm[pearl_mask] - 0.7) / 0.3))
    g_col[pearl_mask] = np.maximum(g_col[pearl_mask], 15 + 230 * ((density_norm[pearl_mask] - 0.7) / 0.3))
    b_col[pearl_mask] = np.maximum(b_col[pearl_mask], 20 + 225 * ((density_norm[pearl_mask] - 0.7) / 0.3))
