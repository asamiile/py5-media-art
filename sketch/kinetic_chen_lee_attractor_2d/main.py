...
def step_chen_lee(a, dt=0.005):
    global x, y, z
    b = -10.0
    c = -3.8
    
    # Euler integration step for the Chen-Lee Attractor
    dx = a * x - y * z
    dy = b * y + x * z
    dz = c * z + x * (y / 3.0)
...
def draw():
    ...
    # Map to screen
    # Chen-Lee typical size: x,y,z roughly in [-20, 20]
    ...
    r_col[mag_mask] = np.maximum(r_col[mag_mask], 5 + 235 * ((density_norm[mag_mask] - 0.3) / 0.4))
    g_col[mag_mask] = np.maximum(g_col[mag_mask], 65 - 55 * ((density_norm[mag_mask] - 0.3) / 0.4))
    b_col[mag_mask] = np.maximum(b_col[mag_mask], 55 + 165 * ((density_norm[mag_mask] - 0.3) / 0.4))
    
    # Gold highlights
    gold_mask = density_norm > 0.7
    r_col[gold_mask] = np.maximum(r_col[gold_mask], 240 + 15 * ((density_norm[gold_mask] - 0.7) / 0.3))
    g_col[gold_mask] = np.maximum(g_col[gold_mask], 10 + 200 * ((density_norm[gold_mask] - 0.7) / 0.3))
    b_col[gold_mask] = np.maximum(b_col[gold_mask], 220 - 180 * ((density_norm[gold_mask] - 0.7) / 0.3))
