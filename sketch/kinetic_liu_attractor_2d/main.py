...
z = np.random.uniform(0.1, 0.2, N).astype(np.float32)

density_buffer = np.zeros((SIZE[1], SIZE[0]), dtype=np.float32)

def step_liu(b, dt=0.005):
    global x, y, z
    a = 10.0
    c = 2.5
    k = 1.0
    ...
    # Euler integration step for the Liu Attractor
    dx = a * (y - x)
    dy = b * x - k * x * z
    dz = -c * z + h * (x ** 2)
    ...
    mask = (np.abs(x_new) > 80) | (np.abs(y_new) > 80) | (np.abs(z_new) > 150) | np.isnan(x_new)
    ...
    z_new[mask] = np.random.uniform(0.1, 0.2, np.sum(mask)).astype(np.float32)
...
def draw():
    ...
    # Map to screen
    # Liu typical size: x,y in [-20, 20], z in [0, 50]
    ...
    b_col = 45 * (density_norm ** 1.5)
    
    # Magenta midtones
    mag_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[mag_mask] = np.maximum(r_col[mag_mask], 30 + 170 * ((density_norm[mag_mask] - 0.3) / 0.4))
    g_col[mag_mask] = np.maximum(g_col[mag_mask], 5 + 45 * ((density_norm[mag_mask] - 0.3) / 0.4))
    b_col[mag_mask] = np.maximum(b_col[mag_mask], 45 + 130 * ((density_norm[mag_mask] - 0.3) / 0.4))
    
    # Gold highlights
    gold_mask = density_norm > 0.7
    r_col[gold_mask] = np.maximum(r_col[gold_mask], 200 + 55 * ((density_norm[gold_mask] - 0.7) / 0.3))
    g_col[gold_mask] = np.maximum(g_col[gold_mask], 50 + 190 * ((density_norm[gold_mask] - 0.7) / 0.3))
    b_col[gold_mask] = np.maximum(b_col[gold_mask], 175 - 100 * ((density_norm[gold_mask] - 0.7) / 0.3))
