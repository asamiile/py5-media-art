...
def step_sprott_i(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz I Attractor
    dx = alpha * y
    dy = x + z
    dz = x + y * y - z
...
def draw():
    ...
    alpha = -0.2 + 0.05 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_i(alpha, dt=0.005)
    ...
    # Map to screen
    # Sprott I typical size: x,y,z in [-5, 5]
    ...
    # Teal midtones
    teal_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[teal_mask] = np.maximum(r_col[teal_mask], 25 + 10 * ((density_norm[teal_mask] - 0.3) / 0.4))
    g_col[teal_mask] = np.maximum(g_col[teal_mask], 20 + 130 * ((density_norm[teal_mask] - 0.3) / 0.4))
    b_col[teal_mask] = np.maximum(b_col[teal_mask], 45 + 105 * ((density_norm[teal_mask] - 0.3) / 0.4))
    
    # Silver highlights
    sil_mask = density_norm > 0.7
    r_col[sil_mask] = np.maximum(r_col[sil_mask], 35 + 190 * ((density_norm[sil_mask] - 0.7) / 0.3))
    g_col[sil_mask] = np.maximum(g_col[sil_mask], 150 + 90 * ((density_norm[sil_mask] - 0.7) / 0.3))
    b_col[sil_mask] = np.maximum(b_col[sil_mask], 150 + 95 * ((density_norm[sil_mask] - 0.7) / 0.3))
