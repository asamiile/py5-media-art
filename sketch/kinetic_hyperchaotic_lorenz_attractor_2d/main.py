...
def step_hyperchaotic_lorenz(d, dt=0.005):
    global x, y, z, w
    ...
    # Euler integration step for the Hyperchaotic Lorenz Attractor
    dx = a * (y - x) + w
    dy = c * x - y - x * z
    dz = x * y - b * z
    dw = -d * x - r * w
...
def draw():
    ...
    for _ in range(3):
        step_hyperchaotic_lorenz(d, dt=0.005)
    ...
    # Map to screen
    # Hyperchaotic Lorenz typical size: x,y in [-40, 40], z in [0, 60]
    ...
    # Lilac midtones
    lilac_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[lilac_mask] = np.maximum(r_col[lilac_mask], 5 + 160 * ((density_norm[lilac_mask] - 0.3) / 0.4))
    g_col[lilac_mask] = np.maximum(g_col[lilac_mask], 35 + 75 * ((density_norm[lilac_mask] - 0.3) / 0.4))
    b_col[lilac_mask] = np.maximum(b_col[lilac_mask], 30 + 195 * ((density_norm[lilac_mask] - 0.3) / 0.4))
    
    # White highlights
    white_mask = density_norm > 0.7
    r_col[white_mask] = np.maximum(r_col[white_mask], 165 + 90 * ((density_norm[white_mask] - 0.7) / 0.3))
    g_col[white_mask] = np.maximum(g_col[white_mask], 110 + 145 * ((density_norm[white_mask] - 0.7) / 0.3))
    b_col[white_mask] = np.maximum(b_col[white_mask], 225 + 30 * ((density_norm[white_mask] - 0.7) / 0.3))
