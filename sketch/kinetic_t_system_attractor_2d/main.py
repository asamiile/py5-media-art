...
def step_t_system(c, dt=0.005):
    global x, y, z
    a = 2.1
    b = 0.6
    
    # Euler integration step for the T System Attractor
    dx = a * (y - x)
    dy = (c - a) * x - a * x * z
    dz = x * y - b * z
...
def draw():
    ...
    # Map to screen
    # T-system typical size: x,y in [-15, 15], z in [0, 40] up to 100
    ...
    r_col[rose_mask] = np.maximum(r_col[rose_mask], 15 + 205 * ((density_norm[rose_mask] - 0.3) / 0.4))
    g_col[rose_mask] = np.maximum(g_col[rose_mask], 45 + 30 * ((density_norm[rose_mask] - 0.3) / 0.4))
    b_col[rose_mask] = np.maximum(b_col[rose_mask], 65 + 75 * ((density_norm[rose_mask] - 0.3) / 0.4))
    
    # Peach highlights
    peach_mask = density_norm > 0.7
    r_col[peach_mask] = np.maximum(r_col[peach_mask], 220 + 35 * ((density_norm[peach_mask] - 0.7) / 0.3))
    g_col[peach_mask] = np.maximum(g_col[peach_mask], 75 + 145 * ((density_norm[peach_mask] - 0.7) / 0.3))
    b_col[peach_mask] = np.maximum(b_col[peach_mask], 140 + 60 * ((density_norm[peach_mask] - 0.7) / 0.3))
