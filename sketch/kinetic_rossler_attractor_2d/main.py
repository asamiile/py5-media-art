...
def step_rossler(a, b, c, dt=0.01):
    global x, y, z
    # Euler integration step for the Rössler attractor
    dx = -y - z
    dy = x + a * y
    dz = b + z * (x - c)
    ...
    z_new[mask] = np.random.uniform(0, 10, np.sum(mask)).astype(np.float32)
...
def draw():
    ...
    # Rössler attractor parameters with continuous modulation
    a = 0.2
    b = 0.2
    c = 5.7 + 0.5 * np.sin(t)
    ...
    for _ in range(3):
        step_rossler(a, b, c, dt=0.01)
        
    # Rotate 3D attractor gently over time
    theta = t * 0.4
    phi = np.sin(t * 1.2) * 0.4
    ...
    # Map to screen
    # Rössler attractor typical size: x,y in [-15, 15], z in [0, 25]
    ...
    # Amethyst midtones
    amy_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[amy_mask] = np.maximum(r_col[amy_mask], 30 + 120 * ((density_norm[amy_mask] - 0.3) / 0.4))
    g_col[amy_mask] = np.maximum(g_col[amy_mask], 0 + 60 * ((density_norm[amy_mask] - 0.3) / 0.4))
    b_col[amy_mask] = np.maximum(b_col[amy_mask], 80 + 120 * ((density_norm[amy_mask] - 0.3) / 0.4))
    
    # Coral Pink highlights
    coral_mask = density_norm > 0.7
    r_col[coral_mask] = np.maximum(r_col[coral_mask], 150 + 105 * ((density_norm[coral_mask] - 0.7) / 0.3))
    g_col[coral_mask] = np.maximum(g_col[coral_mask], 60 + 60 * ((density_norm[coral_mask] - 0.7) / 0.3))
    b_col[coral_mask] = np.maximum(b_col[coral_mask], 200 - 100 * ((density_norm[coral_mask] - 0.7) / 0.3))
