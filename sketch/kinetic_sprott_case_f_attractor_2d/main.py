...
def step_sprott_f(b, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott Case F Attractor
    dx = y + z
    dy = -x + b * y
    dz = x**2 - z
...
def draw():
    ...
    # Sprott F parameter with continuous modulation
    b = 0.5 + 0.05 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_f(b, dt=0.005)
    ...
    # Map to screen
    # Sprott F typical size: x,y,z in [-5, 5]
    ...
    g_col[peach_mask] = np.maximum(g_col[peach_mask], 0 + 155 * ((density_norm[peach_mask] - 0.3) / 0.4))
    b_col[peach_mask] = np.maximum(b_col[peach_mask], 50 + 78 * ((density_norm[peach_mask] - 0.3) / 0.4))
    
    # Rose Gold highlights
    rg_mask = density_norm > 0.7
    r_col[rg_mask] = np.maximum(r_col[rg_mask], 255 - 75 * ((density_norm[rg_mask] - 0.7) / 0.3))
    g_col[rg_mask] = np.maximum(g_col[rg_mask], 155 + 50 * ((density_norm[rg_mask] - 0.7) / 0.3))
    b_col[rg_mask] = np.maximum(b_col[rg_mask], 128 + 100 * ((density_norm[rg_mask] - 0.7) / 0.3))
