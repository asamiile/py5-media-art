...
def step_sprott_o(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz O Attractor
    dx = y
    dy = x - z
    dz = x + x * z + alpha * y
...
def draw():
    ...
    # Map to screen
    # Sprott O typical size: x,y,z in [-5, 5]
    ...
    # Jade midtones
    jade_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[jade_mask] = np.maximum(r_col[jade_mask], 10 + 35 * ((density_norm[jade_mask] - 0.3) / 0.4))
    g_col[jade_mask] = np.maximum(g_col[jade_mask], 45 + 130 * ((density_norm[jade_mask] - 0.3) / 0.4))
    b_col[jade_mask] = np.maximum(b_col[jade_mask], 55 + 85 * ((density_norm[jade_mask] - 0.3) / 0.4))
    
    # Emerald highlights
    em_mask = density_norm > 0.7
    r_col[em_mask] = np.maximum(r_col[em_mask], 45 + 55 * ((density_norm[em_mask] - 0.7) / 0.3))
    g_col[em_mask] = np.maximum(g_col[em_mask], 175 + 80 * ((density_norm[em_mask] - 0.7) / 0.3))
    b_col[em_mask] = np.maximum(b_col[em_mask], 140 - 20 * ((density_norm[em_mask] - 0.7) / 0.3))
