...
def step_sprott_j(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz J Attractor
    dx = 2.0 * z
    dy = alpha * y + z
    dz = -x + y + y * y
...
def draw():
    ...
    alpha = -2.0 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_j(alpha, dt=0.005)
    ...
    # Map to screen
    # Sprott J typical size: x,y,z in [-5, 5]
    ...
    b_col = 115 * (density_norm ** 1.5)
    
    # Rust midtones
    rust_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[rust_mask] = np.maximum(r_col[rust_mask], 15 + 165 * ((density_norm[rust_mask] - 0.3) / 0.4))
    g_col[rust_mask] = np.maximum(g_col[rust_mask], 35 + 35 * ((density_norm[rust_mask] - 0.3) / 0.4))
    b_col[rust_mask] = np.maximum(b_col[rust_mask], 115 - 85 * ((density_norm[rust_mask] - 0.3) / 0.4))
    
    # Copper highlights
    cop_mask = density_norm > 0.7
    r_col[cop_mask] = np.maximum(r_col[cop_mask], 180 + 75 * ((density_norm[cop_mask] - 0.7) / 0.3))
    g_col[cop_mask] = np.maximum(g_col[cop_mask], 70 + 85 * ((density_norm[cop_mask] - 0.7) / 0.3))
    b_col[cop_mask] = np.maximum(b_col[cop_mask], 30 + 35 * ((density_norm[cop_mask] - 0.7) / 0.3))
