...
def step_lorenz_84(F, dt=0.01):
    global x, y, z
    a = 0.25
    b = 4.0
    G = 1.0
    
    # Euler integration step for the Lorenz-84 Attractor
    dx = -a * x - y ** 2 - z ** 2 + a * F
    dy = -y + x * y - b * x * z + G
    dz = -z + b * x * y + x * z
...
def draw():
    ...
    # Map to screen
    # Lorenz-84 typical size: x,y,z roughly in [-4, 4] up to [-10, 10]
    ...
    r_col[turq_mask] = np.maximum(r_col[turq_mask], 35 + 25 * ((density_norm[turq_mask] - 0.3) / 0.4))
    g_col[turq_mask] = np.maximum(g_col[turq_mask], 5 + 185 * ((density_norm[turq_mask] - 0.3) / 0.4))
    b_col[turq_mask] = np.maximum(b_col[turq_mask], 55 + 165 * ((density_norm[turq_mask] - 0.3) / 0.4))
    
    # Platinum highlights
    plat_mask = density_norm > 0.7
    r_col[plat_mask] = np.maximum(r_col[plat_mask], 60 + 195 * ((density_norm[plat_mask] - 0.7) / 0.3))
    g_col[plat_mask] = np.maximum(g_col[plat_mask], 190 + 65 * ((density_norm[plat_mask] - 0.7) / 0.3))
    b_col[plat_mask] = np.maximum(b_col[plat_mask], 220 + 35 * ((density_norm[plat_mask] - 0.7) / 0.3))
