...
def step_sprott_l(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz L Attractor
    dx = y + alpha * z
    dy = 0.9 * x * x - y
    dz = 1.0 - x
...
def draw():
    ...
    alpha = 3.9 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_l(alpha, dt=0.005)
    ...
    # Map to screen
    # Sprott L typical size: x,y,z in [-5, 5]
    ...
    # Sapphire midtones
    sap_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[sap_mask] = np.maximum(r_col[sap_mask], 25 + 15 * ((density_norm[sap_mask] - 0.3) / 0.4))
    g_col[sap_mask] = np.maximum(g_col[sap_mask], 20 + 82 * ((density_norm[sap_mask] - 0.3) / 0.4))
    b_col[sap_mask] = np.maximum(b_col[sap_mask], 65 + 125 * ((density_norm[sap_mask] - 0.3) / 0.4))
    
    # Aquamarine highlights
    aqua_mask = density_norm > 0.7
    r_col[aqua_mask] = np.maximum(r_col[aqua_mask], 40 + 85 * ((density_norm[aqua_mask] - 0.7) / 0.3))
    g_col[aqua_mask] = np.maximum(g_col[aqua_mask], 102 + 150 * ((density_norm[aqua_mask] - 0.7) / 0.3))
    b_col[aqua_mask] = np.maximum(b_col[aqua_mask], 190 + 60 * ((density_norm[aqua_mask] - 0.7) / 0.3))
