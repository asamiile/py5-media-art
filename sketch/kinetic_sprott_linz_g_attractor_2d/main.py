...
def step_sprott_g(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz G Attractor
    dx = alpha * x + z
    dy = x * z - y
    dz = -x + y
...
def draw():
    ...
    alpha = 0.4 + 0.05 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_g(alpha, dt=0.005)
    ...
    # Map to screen
    # Sprott G typical size: x,y,z in [-5, 5]
    ...
    r_col[em_mask] = np.maximum(r_col[em_mask], 15 + 20 * ((density_norm[em_mask] - 0.3) / 0.4))
    g_col[em_mask] = np.maximum(g_col[em_mask], 25 + 155 * ((density_norm[em_mask] - 0.3) / 0.4))
    b_col[em_mask] = np.maximum(b_col[em_mask], 20 + 85 * ((density_norm[em_mask] - 0.3) / 0.4))
    
    # Jade highlights
    jade_mask = density_norm > 0.7
    r_col[jade_mask] = np.maximum(r_col[jade_mask], 35 + 130 * ((density_norm[jade_mask] - 0.7) / 0.3))
    g_col[jade_mask] = np.maximum(g_col[jade_mask], 180 + 75 * ((density_norm[jade_mask] - 0.7) / 0.3))
    b_col[jade_mask] = np.maximum(b_col[jade_mask], 105 + 85 * ((density_norm[jade_mask] - 0.7) / 0.3))
