...
def step_sprott_h(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case H Attractor
    dx = -y + (z ** 2)
    dy = x + a * y
    dz = x - z
...
def draw():
    ...
    # Map to screen
    # Sprott Case H typical size: x,y,z in [-5, 5]
    ...
    r_col[crimson_mask] = np.maximum(r_col[crimson_mask], 5 + 195 * ((density_norm[crimson_mask] - 0.3) / 0.4))
    g_col[crimson_mask] = np.maximum(g_col[crimson_mask], 10 + 10 * ((density_norm[crimson_mask] - 0.3) / 0.4))
    b_col[crimson_mask] = np.maximum(b_col[crimson_mask], 40 - 15 * ((density_norm[crimson_mask] - 0.3) / 0.4))
    
    # Slate highlights
    slate_mask = density_norm > 0.7
    r_col[slate_mask] = np.maximum(r_col[slate_mask], 200 - 85 * ((density_norm[slate_mask] - 0.7) / 0.3))
    g_col[slate_mask] = np.maximum(g_col[slate_mask], 20 + 110 * ((density_norm[slate_mask] - 0.7) / 0.3))
    b_col[slate_mask] = np.maximum(b_col[slate_mask], 25 + 120 * ((density_norm[slate_mask] - 0.7) / 0.3))
