...
def step_tigan(c, dt=0.01):
    global x, y, z
    a = 2.1
    b = 0.6
    
    # Euler integration step for the Tigan Attractor
    dx = a * (y - x)
    dy = (c - a) * x - a * x * z
    dz = x * y - b * z
    ...
    mask = (np.abs(x_new) > 60) | (np.abs(y_new) > 60) | (np.abs(z_new) > 100) | np.isnan(x_new)
...
def draw():
    ...
    z_rot1 = z - 20.0 # Center Z slightly
    ...
    # Map to screen
    # Tigan typical size: x,y in [-20, 20], z in [0, 50]
    ...
    r_col[turq_mask] = np.maximum(r_col[turq_mask], 5 + 65 * ((density_norm[turq_mask] - 0.3) / 0.4))
    g_col[turq_mask] = np.maximum(g_col[turq_mask], 25 + 185 * ((density_norm[turq_mask] - 0.3) / 0.4))
    b_col[turq_mask] = np.maximum(b_col[turq_mask], 60 + 160 * ((density_norm[turq_mask] - 0.3) / 0.4))
    
    # Neon Yellow highlights
    yell_mask = density_norm > 0.7
    r_col[yell_mask] = np.maximum(r_col[yell_mask], 70 + 185 * ((density_norm[yell_mask] - 0.7) / 0.3))
    g_col[yell_mask] = np.maximum(g_col[yell_mask], 210 + 45 * ((density_norm[yell_mask] - 0.7) / 0.3))
    b_col[yell_mask] = np.maximum(b_col[yell_mask], 220 - 150 * ((density_norm[yell_mask] - 0.7) / 0.3))
