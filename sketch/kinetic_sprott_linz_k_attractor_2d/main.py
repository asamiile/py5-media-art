...
def step_sprott_k(alpha, dt=0.01):
    global x, y, z
    # Euler integration step for the Sprott-Linz K Attractor
    dx = x * y - z
    dy = x - y
    dz = x + alpha * z
...
def draw():
    ...
    alpha = 0.3 + 0.05 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_sprott_k(alpha, dt=0.005)
    ...
    # Map to screen
    # Sprott K typical size: x,y,z in [-5, 5]
    ...
    r_col[crim_mask] = np.maximum(r_col[crim_mask], 45 + 175 * ((density_norm[crim_mask] - 0.3) / 0.4))
    g_col[crim_mask] = np.maximum(g_col[crim_mask], 15 + 25 * ((density_norm[crim_mask] - 0.3) / 0.4))
    b_col[crim_mask] = np.maximum(b_col[crim_mask], 20 + 35 * ((density_norm[crim_mask] - 0.3) / 0.4))
    
    # Gold highlights
    gold_mask = density_norm > 0.7
    r_col[gold_mask] = np.maximum(r_col[gold_mask], 220 + 35 * ((density_norm[gold_mask] - 0.7) / 0.3))
    g_col[gold_mask] = np.maximum(g_col[gold_mask], 40 + 175 * ((density_norm[gold_mask] - 0.7) / 0.3))
    b_col[gold_mask] = np.maximum(b_col[gold_mask], 55 + 20 * ((density_norm[gold_mask] - 0.7) / 0.3))
