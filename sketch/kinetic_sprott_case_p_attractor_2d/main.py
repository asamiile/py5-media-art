...
def step_sprott_p(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case P Attractor
    dx = a * y + z
    dy = -x + (y ** 2)
    dz = x + y
...
def draw():
    ...
    # Map to screen
    # Sprott Case P typical size: x,y,z in [-5, 5]
    ...
    g_col = 28 * (density_norm ** 1.5)
    b_col = 32 * (density_norm ** 1.5)
    
    # Rose midtones
    rose_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[rose_mask] = np.maximum(r_col[rose_mask], 25 + 185 * ((density_norm[rose_mask] - 0.3) / 0.4))
    g_col[rose_mask] = np.maximum(g_col[rose_mask], 28 + 75 * ((density_norm[rose_mask] - 0.3) / 0.4))
    b_col[rose_mask] = np.maximum(b_col[rose_mask], 32 + 100 * ((density_norm[rose_mask] - 0.3) / 0.4))
    
    # Mint highlights
    mint_mask = density_norm > 0.7
    r_col[mint_mask] = np.maximum(r_col[mint_mask], 210 - 75 * ((density_norm[mint_mask] - 0.7) / 0.3))
    g_col[mint_mask] = np.maximum(g_col[mint_mask], 103 + 140 * ((density_norm[mint_mask] - 0.7) / 0.3))
    b_col[mint_mask] = np.maximum(b_col[mint_mask], 132 + 80 * ((density_norm[mint_mask] - 0.7) / 0.3))
