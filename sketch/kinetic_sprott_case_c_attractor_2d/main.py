...
def step_sprott_c(a, dt=0.01):
    global x, y, z
    
    # Euler integration step for the Sprott Case C Attractor
    dx = y * z
    dy = x - y
    dz = a - (x ** 2)
...
def draw():
    ...
    # Map to screen
    # Sprott Case C typical size: x,y,z in [-5, 5]
    ...
    r_col[rust_mask] = np.maximum(r_col[rust_mask], 30 + 170 * ((density_norm[rust_mask] - 0.3) / 0.4))
    g_col[rust_mask] = np.maximum(g_col[rust_mask], 35 + 45 * ((density_norm[rust_mask] - 0.3) / 0.4))
    b_col[rust_mask] = np.maximum(b_col[rust_mask], 15 + 15 * ((density_norm[rust_mask] - 0.3) / 0.4))
    
    # Sand highlights
    sand_mask = density_norm > 0.7
    r_col[sand_mask] = np.maximum(r_col[sand_mask], 200 + 45 * ((density_norm[sand_mask] - 0.7) / 0.3))
    g_col[sand_mask] = np.maximum(g_col[sand_mask], 80 + 150 * ((density_norm[sand_mask] - 0.7) / 0.3))
    b_col[sand_mask] = np.maximum(b_col[sand_mask], 30 + 180 * ((density_norm[sand_mask] - 0.7) / 0.3))
