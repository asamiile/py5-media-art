...
def step_genesio_tesi(c, dt=0.005):
    global x, y, z
    a = 1.2
    b = 2.92
    
    # Euler integration step for the Genesio-Tesi Attractor
    dx = y
    dy = z
    dz = -c * x - b * y - a * z + x * x
...
def draw():
    ...
    # Map to screen
    # Genesio-Tesi typical size: x,y,z in [-2, 2] roughly
    ...
    r_col[violet_mask] = np.maximum(r_col[violet_mask], 35 + 100 * ((density_norm[violet_mask] - 0.3) / 0.4))
    g_col[violet_mask] = np.maximum(g_col[violet_mask], 45 - 25 * ((density_norm[violet_mask] - 0.3) / 0.4))
    b_col[violet_mask] = np.maximum(b_col[violet_mask], 55 + 165 * ((density_norm[violet_mask] - 0.3) / 0.4))
    
    # Chartreuse highlights
    chart_mask = density_norm > 0.7
    r_col[chart_mask] = np.maximum(r_col[chart_mask], 135 + 85 * ((density_norm[chart_mask] - 0.7) / 0.3))
    g_col[chart_mask] = np.maximum(g_col[chart_mask], 20 + 235 * ((density_norm[chart_mask] - 0.7) / 0.3))
    b_col[chart_mask] = np.maximum(b_col[chart_mask], 220 - 220 * ((density_norm[chart_mask] - 0.7) / 0.3))
