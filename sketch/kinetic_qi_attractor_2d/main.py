...
def step_qi(c, dt=0.005):
    global x, y, z
    # Euler integration step for the Qi Attractor
    a = 35.0
    b = 8.0/3.0
    ...
    mask = (np.abs(x_new) > 100) | (np.abs(y_new) > 100) | (np.abs(z_new) > 200) | np.isnan(x_new)
...
def draw():
    ...
    # Qi parameter with continuous modulation
    c = 80.0 + 20.0 * np.sin(t)
    ...
    # Map to screen
    # Qi typical size: x,y in [-50, 50], z in [0, 150]
    screen_x = (x_rot2 + 50.0) / 100.0 * SIZE[0]
    screen_y = (y_rot2 + 50.0) / 100.0 * SIZE[1]
    ...
    # Rose midtones
    rose_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[rose_mask] = np.maximum(r_col[rose_mask], 30 + 195 * ((density_norm[rose_mask] - 0.3) / 0.4))
    g_col[rose_mask] = np.maximum(g_col[rose_mask], 20 + 20 * ((density_norm[rose_mask] - 0.3) / 0.4))
    b_col[rose_mask] = np.maximum(b_col[rose_mask], 50 + 60 * ((density_norm[rose_mask] - 0.3) / 0.4))
    
    # Peach highlights
    peach_mask = density_norm > 0.7
    r_col[peach_mask] = np.maximum(r_col[peach_mask], 225 + 30 * ((density_norm[peach_mask] - 0.7) / 0.3))
    g_col[peach_mask] = np.maximum(g_col[peach_mask], 40 + 170 * ((density_norm[peach_mask] - 0.7) / 0.3))
    b_col[peach_mask] = np.maximum(b_col[peach_mask], 110 + 40 * ((density_norm[peach_mask] - 0.7) / 0.3))
