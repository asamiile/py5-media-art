...
def step_wimol_banlue(a, dt=0.005):
    global x, y, z
    
    # Euler integration step for the Wimol-Banlue Attractor
    dx = y - x
    dy = -z * np.sign(x)
    dz = np.abs(x) - a
...
def draw():
    ...
    # Map to screen
    # Wimol-Banlue typical size: x,y,z in [-5, 5] roughly
    ...
    # Amber midtones
    amber_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[amber_mask] = np.maximum(r_col[amber_mask], 15 + 240 * ((density_norm[amber_mask] - 0.3) / 0.4))
    g_col[amber_mask] = np.maximum(g_col[amber_mask], 25 + 160 * ((density_norm[amber_mask] - 0.3) / 0.4))
    b_col[amber_mask] = np.maximum(b_col[amber_mask], 65 - 45 * ((density_norm[amber_mask] - 0.3) / 0.4))
    
    # Pearl highlights
    pearl_mask = density_norm > 0.7
    r_col[pearl_mask] = np.maximum(r_col[pearl_mask], 255)
    g_col[pearl_mask] = np.maximum(g_col[pearl_mask], 185 + 65 * ((density_norm[pearl_mask] - 0.7) / 0.3))
    b_col[pearl_mask] = np.maximum(b_col[pearl_mask], 20 + 235 * ((density_norm[pearl_mask] - 0.7) / 0.3))
