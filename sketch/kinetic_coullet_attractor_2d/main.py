...
def step_coullet(a, dt=0.01):
    global x, y, z
    # Euler integration step for the Coullet Attractor
    b = -1.1
    c = -0.45
...
def draw():
    ...
    # Coullet parameter with continuous modulation
    a = 0.8 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_coullet(a, dt=0.005)
    ...
    # Map to screen
    # Coullet typical size: x,y,z in [-5, 5]
    ...
    # Amethyst base
    r_col = 50 * (density_norm ** 1.5)
    g_col = 0 * (density_norm ** 1.5)
    b_col = 50 * (density_norm ** 1.5)
    
    # Ruby midtones
    ruby_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[ruby_mask] = np.maximum(r_col[ruby_mask], 50 + 170 * ((density_norm[ruby_mask] - 0.3) / 0.4))
    g_col[ruby_mask] = np.maximum(g_col[ruby_mask], 0 + 10 * ((density_norm[ruby_mask] - 0.3) / 0.4))
    b_col[ruby_mask] = np.maximum(b_col[ruby_mask], 50 + 20 * ((density_norm[ruby_mask] - 0.3) / 0.4))
    
    # Crimson highlights
    crim_mask = density_norm > 0.7
    r_col[crim_mask] = np.maximum(r_col[crim_mask], 220 + 35 * ((density_norm[crim_mask] - 0.7) / 0.3))
    g_col[crim_mask] = np.maximum(g_col[crim_mask], 10 + 10 * ((density_norm[crim_mask] - 0.7) / 0.3))
    b_col[crim_mask] = np.maximum(b_col[crim_mask], 70 - 30 * ((density_norm[crim_mask] - 0.7) / 0.3))
