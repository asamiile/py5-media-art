...
def step_newton_leipnik(a, dt=0.01):
    global x, y, z
    b = 0.175
    
    # Euler integration step for the Newton-Leipnik Attractor
    dx = -a * x + y + 10.0 * y * z
    dy = -x - 0.4 * y + 5.0 * x * z
    dz = b * z - 5.0 * x * y
...
def draw():
    ...
    for _ in range(5):
        step_newton_leipnik(a, dt=0.005)
    ...
    # Map to screen
    # Newton-Leipnik typical size: x,y in [-1, 1], z in [-0.5, 0.5] roughly
    screen_x = (x_rot2 + 1.0) / 2.0 * SIZE[0]
    screen_y = (y_rot2 + 1.0) / 2.0 * SIZE[1]
    ...
    g_col = 65 * (density_norm ** 1.5)
    b_col = 65 * (density_norm ** 1.5)
    
    # Coral midtones
    coral_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[coral_mask] = np.maximum(r_col[coral_mask], 15 + 235 * ((density_norm[coral_mask] - 0.3) / 0.4))
    g_col[coral_mask] = np.maximum(g_col[coral_mask], 65 + 65 * ((density_norm[coral_mask] - 0.3) / 0.4))
    b_col[coral_mask] = np.maximum(b_col[coral_mask], 65 + 35 * ((density_norm[coral_mask] - 0.3) / 0.4))
    
    # Cream highlights
    cream_mask = density_norm > 0.7
    r_col[cream_mask] = np.maximum(r_col[cream_mask], 250 + 5 * ((density_norm[cream_mask] - 0.7) / 0.3))
    g_col[cream_mask] = np.maximum(g_col[cream_mask], 130 + 115 * ((density_norm[cream_mask] - 0.7) / 0.3))
    b_col[cream_mask] = np.maximum(b_col[cream_mask], 100 + 135 * ((density_norm[cream_mask] - 0.7) / 0.3))
