...
def step_rikitake(mu, dt=0.01):
    global x, y, z
    # Euler integration step for the Rikitake Attractor
    a = 5.0
...
def draw():
    ...
    # Rikitake parameter with continuous modulation
    mu = 2.0 + 0.1 * np.sin(t)
    
    # Run integration steps
    for _ in range(5):
        step_rikitake(mu, dt=0.005)
    ...
    # Map to screen
    # Rikitake typical size: x,y,z in [-10, 10]
    ...
    r_col[rust_mask] = np.maximum(r_col[rust_mask], 25 + 155 * ((density_norm[rust_mask] - 0.3) / 0.4))
    g_col[rust_mask] = np.maximum(g_col[rust_mask], 25 + 30 * ((density_norm[rust_mask] - 0.3) / 0.4))
    b_col[rust_mask] = np.maximum(b_col[rust_mask], 30 + 10 * ((density_norm[rust_mask] - 0.3) / 0.4))
    
    # Copper highlights
    cop_mask = density_norm > 0.7
    r_col[cop_mask] = np.maximum(r_col[cop_mask], 180 + 75 * ((density_norm[cop_mask] - 0.7) / 0.3))
    g_col[cop_mask] = np.maximum(g_col[cop_mask], 55 + 100 * ((density_norm[cop_mask] - 0.7) / 0.3))
    b_col[cop_mask] = np.maximum(b_col[cop_mask], 40 + 40 * ((density_norm[cop_mask] - 0.7) / 0.3))
