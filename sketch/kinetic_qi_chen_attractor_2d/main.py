...
def step_qi_chen(c, dt=0.002):
    global x, y, z
    
    a = 38.0
    b = 8.0 / 3.0
    
    # Euler integration step for the Qi-Chen Attractor
    dx = a * (y - x) + y * z
    dy = c * x - y - x * z
    dz = x * y - b * z
    ...
    mask = (np.abs(x_new) > 300) | (np.abs(y_new) > 300) | (np.abs(z_new) > 300) | np.isnan(x_new)
...
def draw():
    ...
    c = 80.0 + 10.0 * np.sin(t)
    
    # Run integration steps
    for _ in range(3):
        step_qi_chen(c, dt=0.002)
    ...
    # Map to screen
    # Qi-Chen typical size: x,y,z in [-150, 150]
    ...
    # Charcoal base
    r_col = 25 * (density_norm ** 1.5)
    g_col = 30 * (density_norm ** 1.5)
    b_col = 35 * (density_norm ** 1.5)
    
    # Emerald midtones
    emerald_mask = (density_norm > 0.3) & (density_norm < 0.7)
    r_col[emerald_mask] = np.maximum(r_col[emerald_mask], 25 + 25 * ((density_norm[emerald_mask] - 0.3) / 0.4))
    g_col[emerald_mask] = np.maximum(g_col[emerald_mask], 30 + 170 * ((density_norm[emerald_mask] - 0.3) / 0.4))
    b_col[emerald_mask] = np.maximum(b_col[emerald_mask], 35 + 80 * ((density_norm[emerald_mask] - 0.3) / 0.4))
    
    # Gold highlights
    gold_mask = density_norm > 0.7
    r_col[gold_mask] = np.maximum(r_col[gold_mask], 50 + 205 * ((density_norm[gold_mask] - 0.7) / 0.3))
    g_col[gold_mask] = np.maximum(g_col[gold_mask], 200 + 35 * ((density_norm[gold_mask] - 0.7) / 0.3))
    b_col[gold_mask] = np.maximum(b_col[gold_mask], 115 - 95 * ((density_norm[gold_mask] - 0.7) / 0.3))
